 ...
 *   hasRecap: boolean,
 *   recap: {
 *     overview: { tracks_analyzed, tracks_matched, tracks_unmatched, tracks_ambiguous,
 *                 coverage_pct, last_refreshed_at },
 *     audioProfile: { ...feature percentages },
 *     topGenres: [ { genre, pct } ],
 *     topArtists: [ { artist, track_count } ],
 *     moodDistribution: { ... },
 *     personality: { archetype, tagline, desc },
 *     tasteHighlights: [ { type, text, value } ],
 *   }
 * }
 *
 * tasteHighlights are factual observations derived from the persisted profile.
...
function generateHighlights(profile) {
  ...
  // 1. Strongest audio characteristic (highest % among bounded features)
  const bounded = [
  ...
  return highlights.slice(0, 6); // cap at 6 highlights
}
...
module.exports = async function recap(req, res) {
  ...
  return sendJson(res, 200, { hasRecap: true, recap });
};
