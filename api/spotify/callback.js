...
module.exports = async function callback(req, res) {
  ...
  // The callback may arrive without an existing MusicLens session cookie in
  // some browser flows (e.g. third-party cookie blocking).  We look up the
  // state record directly — the user_id is embedded in the row.
  const sql = getDb();
  let stateRow;
  try {
  ...
  const accountId = spotifyProfile.account_id;
  const spotifyUserId = spotifyProfile.id;
  const displayName = spotifyProfile.display_name || null;

  if (!accountId) {
    return sendJson(res, 502, { error: 'Spotify did not return a valid account_id.' });
  }
  ...
  // Tokens are NOT included in the redirect URL.
