...
/**
 * Load and decrypt the stored Spotify connection for a user.
 * Returns null if no connection exists.
 * NEVER returns decrypted token values outside this module.
 */
 ...
 * Refreshes and re-encrypts if needed. Never logs token values.
 *
 * Throws SpotifyAuthError if the token is revoked or refresh fails.
 */
async function getAccessToken(userId) {
  ...
  // Persist the new access token (and refresh token if Spotify rotated it)
  ...
  if (tokenData.refresh_token) {
    updateFields.refresh_token_encrypted = encrypt(tokenData.refresh_token);
  }
...
/**
 * Fetch a Spotify API endpoint with automatic token refresh.
 * Returns the parsed JSON body.
 * Throws SpotifyAuthError on 401/403, SpotifyRateLimitError on 429,
 * SpotifyApiError on other failures.
 */
...
/**
 * Fetch a user's top tracks across short, medium, and long term ranges.
 * Returns up to 150 unique tracks (50 per time range).
 */
async function fetchTopTracks(userId) {
  ...
  for (const range of ranges) {
    try {
      ...
      // Non-fatal: one time range failing shouldn't abort the whole sync
...
/**
 * Fetch recently played tracks (up to 50 — Spotify's max for this endpoint).
 */
...
/**
 * Fetch saved/liked tracks from the user's Spotify library (up to 50).
 */
...
/**
 * Fetch all available Spotify music data for a user.
 * Returns a deduplicated flat array with source annotations.
 * Deduplication across sources keeps the first-seen source.
 */
async function fetchAllUserMusic(userId) {
  ...
  // Deduplicate across sources — keep all (same track may appear in multiple sources;
  // UNIQUE constraint in user_tracks handles DB-level deduplication per source)
