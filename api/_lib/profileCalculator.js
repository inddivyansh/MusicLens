...
// Features with [0,1] range — converted to percentages for display
...
/**
 * Determine personality archetype from average feature values.
 * Mirrors pipeline/utils/user_profile.py::determine_personality_archetype exactly.
 */
...
/**
 * Calculate the full MusicLens profile from audio feature rows.
 *
 * @param {Array<object>} audioRows — rows from audio_features JOIN tracks JOIN artists
 *   Each row must have: track_id, artist_name, genre_name (may be null),
 *   danceability, energy, loudness, speechiness, acousticness,
 *   instrumentalness, liveness, valence, tempo
 * @param {{ total, matched, unmatched, ambiguous, coverage_pct }} coverageStats
 * @returns {object} Full profile object matching user_profile_data columns
 */
function calculateProfile(audioRows, coverageStats) {
  ...
  // Store as ordered array matching RECOMMENDATION_FEATURES order
  ...
  return {
    ...
    dominant_subgenres: {},        // subgenre data not in track_feature_vectors; can be added in future
